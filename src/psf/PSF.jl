...
@compat const ComponentParams{T} = Optim.Representation{T}
...
function Config(init_params::ComponentParams{Float64}, K::Int;
                verbose::Bool=false,
                ftol::Float64=1e-9,
                grtol::Float64=1e-9)
    ...
    return Config(constraints, ftol, grtol, num_iters, verbose, K,
...
function psf_fit_for_optim!(raw_psf, x_mat, cfg::Config, x::Vector)
    if x != cfg.free_previous_input
        ...
        evaluate_psf_fit!(cfg.bound_params, raw_psf, x_mat, psf_optimizer.bvn_derivs,
...
function psf_fit_for_optim(psf_optimizer::Config, raw_psf, x_mat, psf_params_free_vec::Vector)
    ...
    propagate_derivatives!(to_bound!, bound_result,
                           cfg.free_result, cfg.free_params,
                           psf_optimizer.constraints,
                           psf_optimizer.derivs)
...
function fit_psf(psf_optimizer::Config, raw_psf::Matrix{Float64}, initial_params::Vector{Vector{Float64}})
...
Args:
    ...
    - psf_optimizer: A Config object for the fit.

Returns:
    - A vector of PsfComponents fit to the raw_psf.
"""
function fit_raw_psf_for_celeste{P<:Config}(
...
function fit_raw_psf_for_celeste(raw_psf::Array{Float64, 2}, K::Integer; ftol=1e-9)
    ...
    psf_optimizer = Config(psf_transform, K)
